...
namespace com.hexengine.gear.ecs {
	public partial struct RespondCreateCylinderGeometryPrefabRequestSystem : ISystem {
		private const int SIDE_COUNT = 16;
		...
		void ISystem.OnCreate(ref SystemState state) {
			...
			query = new EntityQueryBuilder(Allocator.Temp)
				.WithAll<CreateCylinderGeometryPrefabRequest>()
				.Build(ref state);
		...
		partial struct Job : IJobEntity {
			...
			void Execute(in Entity entity, [EntityIndexInQuery]int sortKey, RefRO<CreateCylinderGeometryPrefabRequest> request) {
