...
namespace com.hexengine.gear.ecs {
	public partial struct RespondInstantiateBoxRequestSystem : ISystem {
		private EntityQuery query;
		private EntityQuery prefabQuery;

		void ISystem.OnCreate(ref SystemState state) {
			query = new EntityQueryBuilder(Allocator.Temp)
				.WithAll<InstantiateBoxGeometryRequest>()
				.Build(ref state);
		...
		void ISystem.OnUpdate(ref SystemState state) {
			NativeArray<InstantiateBoxGeometryRequest> requests = query.ToComponentDataArray<InstantiateBoxGeometryRequest>(Allocator.TempJob);
		...
		partial struct Job : IJobEntity {
			[ReadOnly] public NativeArray<InstantiateBoxGeometryRequest> requests;
			...
			void Execute(
				...
				foreach(InstantiateBoxGeometryRequest request in requests) {
...
#if UNITY_EDITOR
						commandBuffer.SetName(sortKey, instance, new FixedString64Bytes("Box"));
#endif
					}
				}
			}
