...
namespace com.hexengine.gear.ecs {
	public partial struct RespondCreateCapsuleGeometryPrefabRequestSystem : ISystem {
		...
		void ISystem.OnCreate(ref SystemState state) {
			...
			query = new EntityQueryBuilder(Allocator.Temp)
				.WithAll<CreateCapsuleGeometryPrefabRequest>()
				.Build(ref state);
		...
		partial struct Job : IJobEntity {
			...
			void Execute(in Entity entity, [EntityIndexInQuery]int sortKey, RefRO<CreateCapsuleGeometryPrefabRequest> request) {
				commandBuffer.DestroyEntity(sortKey, entity);

				float height = math.max(request.ValueRO.height, 2.0f * request.ValueRO.radius);
				BlobAssetReference<Collider> geometry = CapsuleCollider.Create(
					new CapsuleGeometry {
						Vertex0 = new float3(0.0f, request.ValueRO.radius, 0.0f),
						Vertex1 = new float3(0.0f, height - request.ValueRO.radius, 0.0f),
