...
namespace com.hexengine.gear.ecs {
	public partial struct RespondInstantiateCylinderRequestSystem : ISystem {
		private EntityQuery query;
		private EntityQuery prefabQuery;

		void ISystem.OnCreate(ref SystemState state) {
			query = new EntityQueryBuilder(Allocator.Temp)
				.WithAll<InstantiateCylinderGeometryRequest>()
				.Build(ref state);
		...
		void ISystem.OnUpdate(ref SystemState state) {
			NativeArray<InstantiateCylinderGeometryRequest> requests = query.ToComponentDataArray<InstantiateCylinderGeometryRequest>(Allocator.TempJob);
		...
		partial struct Job : IJobEntity {
			[ReadOnly] public NativeArray<InstantiateCylinderGeometryRequest> requests;
			...
			void Execute(
				...
				foreach(InstantiateCylinderGeometryRequest request in requests) {
...
#if UNITY_EDITOR
						commandBuffer.SetName(sortKey, instance, new FixedString64Bytes("Cylinder"));
#endif
					}
				}
			}
